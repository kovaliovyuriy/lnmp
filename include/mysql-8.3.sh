...
Install_MySQL83() {
  ...
  if [ "${dbinstallmethod}" == "1" ]; then
    tar xJf mysql-${mysql83_ver}-linux-glibc2.17-x86_64.tar.xz
    mv mysql-${mysql83_ver}-linux-glibc2.17-x86_64/* ${mysql_install_dir}
    sed -i "s@/usr/local/mysql@${mysql_install_dir}@g" ${mysql_install_dir}/bin/mysqld_safe
  elif [ "${dbinstallmethod}" == "2" ]; then
    boostVersion2=$(echo ${boost_ver} | awk -F. '{print $1"_"$2"_"$3}')
    tar xzf boost_${boostVersion2}.tar.gz
    tar xzf mysql-${mysql83_ver}.tar.gz
    pushd mysql-${mysql83_ver}
  ...
  if [ -d "${mysql_install_dir}/support-files" ]; then
    ...
    if [ "${dbinstallmethod}" == "1" ]; then
      rm -rf mysql-${mysql83_ver}-*-x86_64
    elif [ "${dbinstallmethod}" == "2" ]; then
      rm -rf mysql-${mysql83_ver} boost_${boostVersion2}
    fi
  else
